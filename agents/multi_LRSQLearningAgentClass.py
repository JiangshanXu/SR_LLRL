...
class MultiLRSQLearningAgent(Agent):
    ''' Implementation for a CBRS Q Learning Agent '''

    def __init__(self, actions, name="multi-LRS-Q-learning", init_q=None, alpha=0.05, gamma=0.99, epsilon=0.1, explore="uniform", anneal=False, default_q=1.0/(1.0-0.99)):
        # state_n, action_n,
        '''
        Args:
    ...
    def update(self, state, action, reward, next_state):
        ...
        if len(self.state_list) == self.n:
            ...
            for i in reversed(range(self.n)):
                ...
                # prev_q_val = self.get_q_value(self.state_list[i], self.action_list[i])
                # self.q_func[self.state_list[i]][self.action_list[i]] = (1 - self.alpha) * prev_q_val + self.alpha * G
            s = self.state_list.pop(0)
            a = self.action_list.pop(0)
            self.reward_list.pop(0)
            prev_q_val = self.get_q_value(state, action)
            self.q_func[s][a] += self.alpha * (G - self.q_func[s][a])
            # self.q_func[s][a] += self.alpha * (G - self.q_func[s][a])
