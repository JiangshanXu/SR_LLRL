...
class DynaLRSQLearningAgent(Agent):
    ''' Implementation for a CBRS Q Learning Agent '''

    def __init__(self, actions, name="Dyna-LRS-Q-learning", init_q=None, alpha=0.05, gamma=0.99, epsilon=0.1, explore="uniform", anneal=False, default_q=1.0/(1.0-0.99)):
        # state_n, action_n,
        '''
        Args:
    ...
    def update(self, state, action, reward, next_state):
        ...
        for _ in range(self.n_planning):
            if len(self.data_buffer) == 0:
                break
            # state, action, reward, next_state = self.data_buffer.pop(0)
            # use random choice:
            index = random.choice(range(len(self.data_buffer)))
            state, action, reward, next_state = self.data_buffer[index]
