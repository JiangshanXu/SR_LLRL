...
class SRDQLearningAgent(Agent):
    ''' Implementation for a CBRS Q Learning Agent '''

    def __init__(self, actions, name="SR-D-Q-learning", init_q=None, alpha=0.05, gamma=0.99, epsilon=0.1,
                 explore="uniform", anneal=False,
        ...
        # self.dqn_reward_sa = defaultdict(lambda : defaultdict(lambda: 0))
        # dqn_reward_sa takes list as state, and index as action, so how to create this matrix?
        self.dqn_reward_sa = {}
    ...
    def act(self, state, reward, explore=True, learning=True):
        ...
        # add reward heuristic:
        # if self.prev_state is not None:

        if self.prev_state is not None:
            # reward = reward + self.reward_sa[self.prev_state][self.prev_action]
            # reward = reward + self.dqn_reward_sa[self.prev_state][self.prev_action]
            # make tuple:
            state_action_key = (tuple(self.prev_state), self.prev_action)
            if state_action_key in self.dqn_reward_sa:
                print('sa', state_action_key, 'reward', reward, 'heuristic', self.dqn_reward_sa[state_action_key])
                reward = reward + self.dqn_reward_sa[state_action_key]
        ...
        if learning and self.replay_buffer.size() > self.minimal_buffer_size:
            ...
            transition_dict = {'states': b_state, 'actions': b_action, 'rewards': b_reward, 'next_states': b_next_state,
                               'dones': b_done}
        ...
        if np.random.random() < self.epsilon:
            ...
            state = state.tolist()
        # state can not be tensor, let it be list:
    ...
    def _compute_max_qval_action_pair(self, state):
        ...
        assert not state.is_terminal()
        state = state.get_data()
        return self.q_net(torch.tensor(state, dtype=torch.float).to(self.device)).max().item(), self.actions[
            self.q_net(torch.tensor(state, dtype=torch.float).to(self.device)).argmax().item()]
    ...
    def _compute_count_reward(self):
        for x in self.count_sa:
            for y in self.count_sa[x]:
                ...
                # y from str to index:
                index_y = self.actions.index(y)
                list_x = x.get_data()
                state_action_key = (tuple(list_x), index_y)
                # if state_action_key in self.dqn_reward_sa:
                #     self.dqn_reward_sa[state_action_key] += reward
                # else:
                #     self.dqn_reward_sa[state_action_key] = reward
                self.dqn_reward_sa[state_action_key] = (1 - self.gamma) * (
                    (self.count_sa[x][y] / self.count_s[x])) * self.default_q * 0.07

                # self.dqn_reward_sa[x.get_data()][index_y] = (1- self.gamma) * ((self.count_sa[x][y] / self.count_s[x])) * self.default_q
    ...
    def get_q_value(self, state, action):
        ...
        return self.q_net(torch.tensor(state, dtype=torch.float).to(self.device)).gather(1, torch.tensor([action]).to(
            self.device)).item()
