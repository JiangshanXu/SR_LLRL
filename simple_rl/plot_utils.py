...
def averaged_lifelong_plot(
        ...
        figure_title=None): # 'lifelong-four_room_h-11_w-11'
    ...
    for i in range(n_agents):
        ...
        for j in range(1, n_episodes + 1):
            tr_norm, dr_norm = 1., 1.
            if norm_ag is not None and which_norm_ag in [0, 1]: # always none.
                df = dfs[norm_ag]
            ...
            df = dfs[i].loc[dfs[i]['episode'] == j]  # choose data where episode =1,也就是每个任务的第一个episode
            df = df.loc[df['task'] <= n_tasks]  # only select tasks <= n_tasks,任务数量砍到一个上限。

            tre_i.append(mean_confidence_interval(df['return'] / tr_norm, confidence)) # tre_i统计的是每一个episode的平均reward.
            dre_i.append(mean_confidence_interval(df['discounted_return'] / dr_norm, confidence))
        tre.append(tre_i)  # 这里收集的就是不同agent的每个episode的reward.
        dre.append(dre_i)
        ...
        for j in range(1, n_tasks + 1):
            ...
            trt_i.append(mean_confidence_interval(df['return'] / tr_norm, confidence))  # 收集的是一个task的平均reward.
            drt_i.append(mean_confidence_interval(df['discounted_return'] / dr_norm, confidence))  # 收集的是一个task的平均reward.
        trt.append(trt_i)
        drt.append(drt_i)
    ...
    # _lgd = True
    # plot(path, pdf_name = output_dir + '-Episode', agents=agents, x=x_e, y=tre, y_lo=tre_lo, y_up=tre_up,
    ...
    _lgd_btm = True
    _cst = False
    the_minus_third = path.split('/')[-3]
    splited =path.split('/')
    minus_3 = splited[-3]
    minus_2 = splited[-2]
    #  concate:
    minus_3 = minus_3 + '-' + minus_2
    plot(path, pdf_name = output_dir + minus_3, agents=agents, x=x_t, y=trt, y_lo=trt_lo, y_up=trt_up,
...
def lifelong_plot(
    ...
    dfs = []
    for agent in agents:
        agent_path = csv_path_from_agent(path, agent) # './results/Alifelong-four_room_h-11_w-11-q-learning-vs_task/Ideal_result.csv'
        dfs.append(pandas.read_csv(agent_path))

    averaged_lifelong_plot(dfs, agents, path, output_dir, n_tasks, n_episodes, confidence, open_plot, plot_title,
...
def plot(
    ...
    plt.rcParams.update({'font.size': FONT_SIZE})  # default: 10
    if latex_rendering:
    ...
    if agents is None:
        ...
        n_curves = len(agents)
        labels = [_format_label(str(agent), latex_rendering) for agent in agents]

    # x-cut
    if x_cut is not None:  # jump
        x = x[:x_cut]
    ...
    for i in range(n_curves):
        ...
        if plot_markers:
            plt.plot(_x, y[i], label=labels[i], marker=markers[i % len(markers)], markersize=4, markevery=marke_every)
        else:
    ...
    if plot_legend:
        ...
        #     ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.16), fancybox=False, shadow=False, fontsize=14,
        #               ncol=2, columnspacing=10)
        # else:
