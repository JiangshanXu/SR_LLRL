...
class Experiment(object):
    ...
    def __init__(self,
        ...
        if vs_task:
            self.exp_directory = Experiment.RESULTS_DIR + self.name +self.detail_name+ "-" + alg + "-vs_task"
        else:
            self.exp_directory = Experiment.RESULTS_DIR + self.name + self.detail_name+"-" + alg + "-vs_episode"
    ...
    def lifelong_save(self, agent, init, instance_number=None, data=None):
        '''
        ...
        '''
        full_path = os.path.join(self.exp_directory, str(agent)) + "_result.csv"
        if init:
