...
pygame.display.set_caption("Dyna-Q Visualization")
...
arrows = {
    (2, 9): (1, 0),  # Example: move right
    (3, 9): (1, 0),
    (4, 9): (0, -1),
    (4, 8): (0, -1),  # Example: move up
    (4, 7): (1, 0),
    (5, 7): (1, 0),
    (6, 7): (1, 0),
    (7, 7): (1, 0),
    (8, 7): (1, 0),  # Example: move right
    (9, 7): (0, -1),
}
...
def draw_arrows():
    ...
    # Randomly choose positions to update (Dyna-Q simulation)
    # num_updates = random.randint(7, 1)  # Random number of updates (for demonstration)
    # update_positions = random.sample(path_taken, num_updates)
    update_positions = [(9, 7)]
...
while running:
    ...
    if keys[pygame.K_s]:
        ...
        pygame.image.save(screen, f"grid_env_sarsa_{time_format}.png")
        print("saved")
