...
def parse_args():
    ...
    parser.add_argument("-mdp_class", type=str, default="four_room", nargs='?',
...
def main(open_plot=True):
    experiment_name = "four_room_normal_experiment"
    is_ablation = True
    # Environment setting
    episodes = 200
    steps = 200
    ...
    # mdp_distr = make_rescue_grid_world_distribution(obs_num=obstacle_num,patient_count=patient_count,mdp_num=40)
    ...
    if baselines == "q":
        ...
        dyna_sr_llrl = DynaLRSQLearningAgent(actions, alpha=lrate, epsilon=eps, gamma=gamma, default_q=vmax, name="SR_dyna")
        # agents
        ...
        sarsa_lambda_sr_llrl = SarsaLambdaLRSQLearningAgent(actions, alpha=lrate, epsilon=eps, gamma=gamma, default_q=vmax, name="SR_Sarsa_λ")
        sarsa_lambda2_sr_llrl = SarsaLambda2LRSQLearningAgent(actions, alpha=lrate, epsilon=eps, gamma=gamma, default_q=vmax, name="SR_sarsa_λ2")
        # agents = [sarsa_lambda_sr_llrl,multi_sr_llrl,SR_LLRL,dyna_sr_llrl]
        agents = [sarsa_lambda_sr_llrl,multi_sr_llrl,SR_LLRL,dyna_sr_llrl]
        # agents = [SR_LLRL]
        # agents = [Ideal, MaxQinit, SR_LLRL, Baseline,DQN]
    ...
    run_agents_lifelong(agents, mdp_distr, vs_task=vs_task, samples=samples, episodes=episodes, steps=steps,
                        ...
                        experiment_detail_name=experiment_name
                        )
