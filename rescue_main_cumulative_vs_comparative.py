...
def main(open_plot=True):
    set_experiment_content = "sarsa_lambda_SR Cumulative vs sarsa_lambda_SR Replacing"
    is_ablation = False
    # Environment setting
    episodes = 200
    steps = 200
    ...
    if baselines == "q":
        ...
        sarsa_lambda_sr_llrl = SarsaLambdaLRSQLearningAgent(actions, alpha=lrate, epsilon=eps, gamma=gamma, default_q=vmax, name="sarsa_lambda_SR Cumulative")
        sarsa_lambda2_sr_llrl = SarsaLambda2LRSQLearningAgent(actions, alpha=lrate, epsilon=eps, gamma=gamma, default_q=vmax, name="sarsa_lambda_SR Replacing")
