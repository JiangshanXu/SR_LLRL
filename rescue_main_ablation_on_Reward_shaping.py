...
from agents.dynaQLearningAgentClass import DynaQLearningAgent
from agents.multi_QLearningAgentClass import MultiQLearningAgent
from agents.sarsa_lambdaQLearningAgentClass import SarsaLambdaQLearningAgent
...
def main(open_plot=True):
    set_experiment_content = "Ablation_on_reward_shaping"
    is_ablation = False
    # Environment setting
    episodes = 200
    steps = 200
    ...
    if baselines == "q":
        ...
        dyna_sr_llrl = DynaLRSQLearningAgent(actions, alpha=lrate, epsilon=eps, gamma=gamma, default_q=vmax,
                                             name="dyna_SR")
        # agents
        ...
        sarsa_lambda_sr_llrl = SarsaLambdaLRSQLearningAgent(actions, alpha=lrate, epsilon=eps, gamma=gamma,
                                                            default_q=vmax, name="SR_sarsa_λ")

        sarsa_lambda = SarsaLambdaQLearningAgent(actions, alpha=lrate, epsilon=eps, gamma=gamma, name="sarsa_λ")
        dynaQ = DynaQLearningAgent(actions, alpha=lrate, epsilon=eps, gamma=gamma, name="dynaQ")
        multiQ = MultiQLearningAgent(actions, alpha=lrate, epsilon=eps, gamma=gamma, name="Sarsa(5)")
        sarsa_lambda2_sr_llrl = SarsaLambda2LRSQLearningAgent(actions, alpha=lrate, epsilon=eps, gamma=gamma,
                                                              default_q=vmax, name="sarsa_lambda_SR Replacing")
        ...
        # agents = [SR_LLRL]
        # agents = [sarsa_lambda_sr_llrl,sarsa_lambda2_sr_llrl]
        agents = [sarsa_lambda_sr_llrl, sarsa_lambda, dynaQ, multiQ]
    ...
    run_agents_lifelong(agents, mdp_distr, vs_task=vs_task, samples=samples, episodes=episodes, steps=steps,
                        ...
                        experiment_detail_name=set_experiment_content
                        )
